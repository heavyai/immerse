...
export const buildBoxPlotQuery = ({
  ...
  querySpec: VegaBoxPlotChartQuerySpec
  layerIndex: number
  minmax?: MinMax
  ...
  const sizeMeasureExprs = querySpec.sizeMeasures.map(
    (sizeMeasure) =>
      sizeMeasure.type === TIME_LAG_EXPRESSION_TYPE
        ? buildTimeLagExpr(sizeMeasure, minmax, numTopNGroups, querySpec)
        : sizeMeasure.column.value // measureToExprStr(sizeMeasure)
  )

  const withSubqueries: Record<string, string> = {}

  // All the things + params we need
  const boxPlotAggs = [
    ["q1", "APPROX_PERCENTILE", 0.25],
    ["q3", "APPROX_PERCENTILE", 0.75],
  ...
  // There can only be one measure I believe....
  // so we can probably remove the measure{idx} stuff later
  const select = sizeMeasureExprs
    .filter((e) => e !== null)
    .map((sizeMeasureExpr, index) =>
      boxPlotAggs.map((agg) => {
        if (agg.length > 2) {
          return `${agg[1]}(${sizeMeasureExpr}, ${agg
            .slice(2)
            .join(",")}) as measure${index}_${agg[0]}`
        } else {
          return `${agg[1]}(${sizeMeasureExpr}) AS measure${index}_${agg[0]}`
        }
      })
    )
    .flat()

  // add hint for query caching; this is needed so we don't get different results
  // for q1 and q3 from APPROX_PERCENTILE every render, otherwise measureDomainSelector
  // can cause an infinite render loop as getBoxPlotAutoScaling will calculate
  // different extents on every run and fail the equality check
  if (select.length > 0) {
    select[0] = `/*+ keep_result */ ${select[0]}`
  }
  ...
  // TODO: Re-add the layerIndex checks when we add layer support
  // if (layerIndex && layerIndex !== 0 && sortColumnName === "measureColor") {
  //   colorMeasureAlias = "measure0"
  const aggAlias =
    querySpec.centerLineType === BoxPlotCenterLineType.MEAN ? "avg" : "median"
  const colorMeasureAlias =
    sortColumnName === "measure0" ? `measure0_${aggAlias}` : sortColumnName
  ...
  if (
    ...
    // TODO: Removed this, not sure how we should order these things....
    // Maybe there's some data/format settings that can determine this?
    // ${sql.orderBy(
...
export const buildOutliersQuery = ({
  querySpec
}: {
  querySpec: VegaBoxPlotChartQuerySpec
}) => {
  const dimensions = querySpec.baseDimensions
  const sizeMeasureExprs = querySpec.sizeMeasures.map(
    (sizeMeasure) => sizeMeasure.column.value
  )
  const baseDimension = dimensions[0].column.value
  const dataSource = querySpec.table

  const withSubqueries: Record<string, string> = {}

  const percentileExpr = (measure: string) => sql`
    APPROX_PERCENTILE(${measure}, 0.25) - 1.5 * (APPROX_PERCENTILE(${measure}, 0.75) - APPROX_PERCENTILE(${measure}, 0.25)) AS measure0_lower_bound,
    APPROX_PERCENTILE(${measure}, 0.75) + 1.5 * (APPROX_PERCENTILE(${measure}, 0.75) - APPROX_PERCENTILE(${measure}, 0.25)) AS measure0_upper_bound
  `

  const rankExpr = (measure: string, dimension: string) => sql`
    ${measure} AS measure0,
    ROW_NUMBER() OVER (
      PARTITION BY ${dimension}
      ORDER BY ${measure} ASC NULLS LAST
    ) AS measure0_bottom_rank,
    ROW_NUMBER() OVER (
      PARTITION BY ${dimension}
      ORDER BY ${measure} DESC NULLS LAST
    ) AS measure0_top_rank
  `
  ...
  const filterDataExpressions = new Set()
  for (const af of querySpec.appliedFilters) {
    if (af.filter) {
      getDataExpressionsForFilter(af.filter, filterDataExpressions)
    }
  }

  // Subquery: threshold - Calculate upper/lower outlier thresholds per measure
  withSubqueries.threshold = sql`
    ${sql.select([
      sql`${baseDimension} AS dimension0`,
      ...sizeMeasureExprs.map(percentileExpr)
    ])}
    ${sql.from(dataSource)}
    ${sql.where(
      sql.and([
        ...sizeMeasureExprs.map((measure) => sql`${measure} IS NOT NULL`),
        ...where
      ])
    )}
    ${sql.groupBy([baseDimension])}
  `

  // Subquery: ranks - Compute ranking for each measure
  withSubqueries.ranks = sql`
      ${sql.select([
        sql`${baseDimension} AS dimension0`,
        [...filterDataExpressions].join(", "),
        ...sizeMeasureExprs.map((m) => rankExpr(m, baseDimension))
      ])}
      ${sql.from(`${dataSource}`)}
      INNER JOIN threshold
          ON ${baseDimension} = threshold.dimension0
          AND (${sizeMeasureExprs
            .map(
              (measure) => `
                ${measure} > threshold.measure0_upper_bound
                OR ${measure} < threshold.measure0_lower_bound
              `
            )
            .join(" OR ")})
      ${sql.where(sql.and(where))} 
  ...
  // Final Query: Select outliers based on rank filtering
  return sql`
    ${sql.with(withSubqueries)}
    ${sql.select(["*"])}
    ${sql.from("ranks")}
    ${sql.where(
      ...sizeMeasureExprs.map(
        (_, index) =>
          `measure${index}_bottom_rank <= 10 OR measure${index}_top_rank <= 10`
      )
    )}
  `
}
...
// Get min max first
// Get domain values as well

export const buildViolinPlotQuery = ({
  querySpec,
  minmax,
  domainValues
}: {
  ...
  const violinDistributionPrecision =
    querySpec.violinDistributionPrecision ?? DEFAULT_VIOLIN_PRECISION
  const nullDimensionsEnabled =
    !querySpec.binSettings && querySpec.nullDimensionsEnabled
  let sortColumnName = querySpec.sortColumn.col.name
  ...
  if (min === max) {
    throw new Error(
      `Failed to generate box plot, min and max values of ${querySpec.sizeMeasures[0].column.value} are the same. `
    )
  }
  ...
  // There can only be one measure I believe....
  // so we can probably remove the measure{idx} stuff later
  const BUCKET_NUMBER = "bucketNumber"
  const BUCKET_COUNT = "bucketCount"
  const COLOR_DIMENSION = "dimensionColor"
  const baseDimension = dimensions[0].column.value
  const select = [
    `count(*) as ${BUCKET_COUNT}`,
    `CASE
          WHEN ${baseDimension} IN (
            ${Array.from(domainValues)
              .filter((dv) => dv?.length)
              .map((dv) => sql.escape(dv))
              .join(", ")}
          ) THEN ${baseDimension}
        END AS ${COLOR_DIMENSION} 
    `,
    // ELSE "All Other" works if we want to bucket those like that
    sizeMeasureExprs.map(
      (sizeMeasureExpr) =>
        `CASE
          WHEN ${sizeMeasureExpr} >= ${max} THEN ${violinDistributionPrecision}
          ELSE  WIDTH_BUCKET(${sizeMeasureExpr}, ${min}, ${max}, ${violinDistributionPrecision})
        END - 1 AS ${BUCKET_NUMBER}`
    )
  ].flat()
  ...
  sizeMeasureExprs.forEach((measureExpression) => {
    where.push(
      `(
        ${measureExpression} IS NOT NULL
        AND  ${measureExpression} >= ${min}
        AND  ${measureExpression} <= ${max}
      )`
    )
  })

  const groupBy = [BUCKET_NUMBER, COLOR_DIMENSION]
  const having: string[] = [
    sql.and([
      `${BUCKET_NUMBER} >= 0`,
      `${BUCKET_NUMBER} < ${violinDistributionPrecision}`
    ]),
    `${BUCKET_NUMBER} IS NULL`
  ]
  ...
  return sql`
    ...
    ${sql.where(sql.and(where))}
    ${sql.groupBy(groupBy)}
    ${sql.having(sql.and("dimensionColor IS NOT NULL", sql.or(having)))}
  `
}
