...
export function distance_in_meters(fromlon, fromlat, tolon, tolat) {
  const latitudeArc = (fromlat - tolat) * HeavyAIDraw.Math.DEG_TO_RAD
  const longitudeArc = (fromlon - tolon) * HeavyAIDraw.Math.DEG_TO_RAD
  ...
  const tmp =
    Math.cos(fromlat * HeavyAIDraw.Math.DEG_TO_RAD) *
    Math.cos(tolat * HeavyAIDraw.Math.DEG_TO_RAD)
  return (
...
export function conv900913To4326(out, coord) {
  return HeavyAIDraw.Point2d.set(
    out,
