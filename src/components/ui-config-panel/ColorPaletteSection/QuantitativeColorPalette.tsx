...
import { MAX_NUM_CONTINUOUS_SWATCHES } from "components/ui-config-panel/constants"

/**
 * Quantitative color palette editor
 *
 * Quantitative color palettes are generally seen when the user has a color
 * measure. They appear as a gradient in color pickers around Immerse. They are
 * represented by an array of string arrays, like:
 *
 * [
 *  ["#ffffff", "#000000"],
 *  ["#ffff00", "#00fffff"]
...
interface Props {
  label: string
  // The current group of quantitative color palettes
  colorPalettes: NestedColorPalette
  // Sets all quantitative color palettes
  setColorPalettes: (colorPalette: NestedColorPalette) => void
}

const QuantitativeColorPalette: FC<Props> = ({
  label,
  colorPalettes,
  setColorPalettes
}) => {
  // The index of the quantitative palette currently being edtied
  ...
  const palettes = colorPalettes.map((palette, i) => {
    ...
    const classNames = cx("quantitative-palette", {
  ...
  const hasMaxNumSwatches =
    selectedColorPalette.length >= MAX_NUM_CONTINUOUS_SWATCHES

  return (
    <div className="config-ui-color-palette-quantitative">
      <div className="color-palette-label">{label}</div>
      <div
        className="quantitative-palette-preview-container"
        onClick={openPalettePopup}
      >
        <div
          className="quantitative-palette"
          style={{
            background: `linear-gradient(to right, ${colorPalettes[currentPaletteIndex]})`
          }}
        />
        <div className="quantitative-palette-arrow" />
      </div>
      <Popover isOpened={isPalettePopupOpen} onClose={closePalettePopup}>
        <div className="quantitative-palette-popover">
...
export default QuantitativeColorPalette
