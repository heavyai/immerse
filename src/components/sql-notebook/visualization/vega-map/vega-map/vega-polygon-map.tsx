...
import { PolygonMapSettings } from "../types"
import { useGeometryBounds } from "../hooks/use-geometry-bounds"

export const VegaPolygonMap = ({
  mapId,
  chartSettings,
  setLoading,
  showLegend = true
}: IVegaMap) => {
  const chartKey = LAYER_TYPE.POLYGON
  const [viewState, setViewState] = useState(INITIAL_VIEW_STATE)
  const { vegaSpec, error: vegaError, loading: vegaLoading } = useVegaChart(
    chartKey,
    chartSettings as PolygonMapSettings,
    viewState
  )
  ...
  const popupFields = useMemo(
    () => [chartSettings.colorField?.field].filter(Boolean),
    [chartSettings]
  )
