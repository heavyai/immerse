...
xdescribe("number chart recording", () => {
  it("number chart recording", async () => {
    ...
    try {
      ...
      const mocksFile = "number.queries.json"
...

      {
        ...
        const val = await page.$eval(
          "#chart1 > div.number-chart-wrapper > span",
          (e) => {
            return e.innerHTML
          }
        )
      ...
      await checkForModal(page)
      await page.waitForSelector(
        '.dashboard-container > .chart-editor-container > [data-testid="chart-editor-left-panel"] > div > .data-selector-container'
      )
      await page.click(
        '.dashboard-container > .chart-editor-container > [data-testid="chart-editor-left-panel"] > div > .data-selector-container'
      )
      ...
      await checkForModal(page)
      await page.waitForSelector(
        ".selector-pill-wrapper > .selector-pill > .drag-area > #measure-0 > .selector-label-ellipse"
      )
      await page.click(
        ".selector-pill-wrapper > .selector-pill > .drag-area > #measure-0 > .selector-label-ellipse"
      )
      ...
      await checkForModal(page)
      await page.waitForSelector(
        '[data-testid="autocomplete-dropdown"] > .autocomplete-dropdown-list > [data-testid="autocomplete-dropdown-item"]:nth-child(3) > .autocomplete-dropdown-item-content > .value'
      )
      await page.click(
        '[data-testid="autocomplete-dropdown"] > .autocomplete-dropdown-list > [data-testid="autocomplete-dropdown-item"]:nth-child(3) > .autocomplete-dropdown-item-content > .value'
      )

      {
        await waitUntilHTMLRendered(page)
        await checkForModal(page)
        const val = await page.$eval("#chart1 > div > span", (e) => {
          return e.innerHTML
        })
        if (val !== "7.25") {
          throw new Error(
            `Could not continue, invalid count: "${val}" !== "7.25"`
          )
        }
      }
